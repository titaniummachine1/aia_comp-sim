...
- Game-truth policy readout (2026-09-12, Phase B): `scripts/read_battery_from_game.py` reads the game's TimePlot export of `semantics_battery.txt` (v0.14 natural probe; `parse_timeplots` locale-comma rule). The game ran the battery and exported all 46 `B.*` channels: div/mod-by-zero = **IEEE inf/nan** (A1 correct), `ConditionalSetFloat` unwired-false = **previous-tick HOLD**, unwired input = 0, and **all 16 `Operation` indices** match. **New game-sourced open items:** `ConditionalSetVector3` unwired-false = **0, not hold** (VM holds — likely wrong) and Bool→Float coercion reads **0** in the game (`as_float(Bool)`=1.0 may be wrong). Sim NOT changed on their account. Details: HANDOFF §17.
