...
- Per-tick channel diff RUN (2026-09-12, Phase C): `scripts/channel_diff.py` (rewritten: GAME/SIM/Δ table, `--env`, `--json`) compares sim graph channels (`tennis_tournament --trace` + `AIA_TRACE_CHANNELS=1` → `hch`) with the game's native export of the same graph. Biggest gap: **`ChargePct` (game 0.28, sim 0.00)** → the existing `swing_hold_gate` closes it (0.25) and is now the **default** (`AIA_SWING_MODEL=legacy/off` restores the old default; lib 168/0, battery PASS, cert 12/14). Open: `SwingHeld` (sim 0.09 vs game 0.54), `Chase`/`Mode` ~2× overshoot, `Bounced`/`Incoming` overshoot, `BallX` sign flip. Details: HANDOFF §20. Scale-up (§20b): `run_sim_seed_sweep.py` also accepts tournament rows (`game_tournament_results*.jsonl`; first server unknown → hold skipped); replaying the 78-match game set gives sim home-win **55%** vs game **58%**, **TV distance 0.078**. Compiler↔VM guard: `tests/compiler_corpus_parity.rs` lowers every committed `aia_graphc/examples/*.txt` and asserts no `Unimplemented` node (8 sound).
- Phase F4 (2026-09-12): the v0.15f event fixture now RUNS — `metadata_event_capture_group`/`..._group_matches`/`..._restore_group` + the `V014ServeFieldSpec` variants now tolerate version-removed fields (the same `metadata_trace_field_optional` allowlist the trace writer used); `build_paritymod_event_v015.ps1` builds the v0.15 event probe and `emit_rust_fixtures.py` gained `--out=<dir>`. Captured on v0.15f: `curve` **29/29 complete**, `serve_direct` 26, `simulate` 13, `nth_landing` 10; the SHOT fixture's own snapshot initially failed because `v014_shot_ball_fields` marked the three v0.15-removed ball cache fields `required:1` — flipped to optional, after which `shot_fixture` is **complete 210/210 cases / 840/840 calls**. Full corpus in `tests/fixtures/tennis-v015/`. **Result: the v0.15 shot solver is IDENTICAL to v0.14** (`tennis_v014_solver_parity`: both 200/210 within 0.25 m/s, max_err 23.6888, same worst `fallback` row) → no v0.15 solver re-pin needed. Only `getter_items` remains (needs the capture post-graph-load). Details: HANDOFF §21.
- Phase C first-server-matched sweep (2026-09-12): `restart_sweep.py --seeds 1-16 --points 8` (one launch, restarted per seed) + the chained sim replay/score. **Exact per-match parity 11/17 = 64.7%** once the first server matches (vs 32.1% without), **server-hold 53% vs 53%**; residual is strength (game 5.6-1.0 vs sim 3.8-2.1, TV 0.20) → the sim under-rates the stronger bot (the §20 rally-quality gaps). Data: `modhost/restart_sweep.jsonl` + `data/tennis/sim_seed_sweep.jsonl`. Details: HANDOFF §22.
- Simulator hub / v0.15 target (2026-09-12): `GameVersion::TennisV015` + `GameSpec::tennis_v015()` (latest tennis); `tennis_v014()` explicit. v0.15's node ABI is v0.14-identical, so it reuses the label table; the sole world-model delta is the **swept (frame-interpolated) ball contact**, gated by `GameVersion::interpolates_ball()` + `TennisWorld::for_spec` (default `new()` stays v0.14). The time-to-ground cache needs no change (the sim computes predictions fresh). Adding a game/version recipe: `docs/GAME_VERSIONS.md` "Simulator hub". Lib 169/0.
- Fair tick-by-tick comparison (2026-09-12, `scripts/tick_diff.py`): replays the game's own (seed, first server) and diffs `v44_*` sample-by-sample. First result: the **serve-setup window** is the first gap — the game runs the graph before the ball is placed (ball `0`, players at spawn: home −14.84 / away 8.23) while the sim teleports the server to its stance (14.917) at tick 0; the sim's serve setup is ~4× longer (contact ~tick 100 vs game ~24); serve-phase aim agrees for ~39 ticks then drifts. The sim's shot deviation is **zero-mean uniform fatigue scatter with no contact-timing term**, so the "bell-curve centre" question needs a new **contact-grading fixture** (`GradeRallyContact`/`EvaluateHitAccuracy`, uncaptured). Details: HANDOFF §23/§23b.
