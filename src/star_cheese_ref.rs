...
fn blocked_fraction_lite(me: Vec2, my_stam: f32, threats: &[Threat], hold: f32, r_int: f32) -> f32 {
    // Same ordering as the arc integral, without acos: how deep each opponent
    // has pushed inside the annulus where it can block anything at all. Zero
    // outside hold + r_int, growing as it closes, and monotone in the arc it
    // denies — which is all a "who is freest" comparison needs.
    ...
    for t in threats {
        ...
        let depth = (hold + r_int) - d;
        if depth > 0.0 {
            score += depth;
        }
    }
    (score / (4.0 * (hold + r_int))).min(1.0)
}

fn freedom(lite: bool, me: Vec2, s: f32, th: &[Threat], hold: f32, r: f32) -> f32 {
    if lite { blocked_fraction_lite(me, s, th, hold, r) } else { blocked_fraction(me, s, th, hold, r) }
}
...
fn intercept_point(me: Vec2, ball: Vec2, bvel: Vec2, decel: f32, p: &SimParams, dt: f32, lite: bool) -> Vec2 {
    // A graph cannot loop, so `lite` unrolls to 12 fixed sample times spanning
    // the same horizon instead of stepping every tick.
    let (n, stride) = if lite { (12, 10.0) } else { (160, 1.0) };
    for k in 0..n {
        let t = k as f32 * dt * stride;
...
pub fn decide(
    ...
    let intercept_on = true;
        ...
        let ball_blocked = match carrier {
            Some((TeamId::Home, id)) => {
                let p = w
                    ...
                    .unwrap();
                freedom(lite, p.pos, p.stamina, &threats, hold, r_int)
            }
            ...
            let (bvel, decel) = match carrier {
                Some((TeamId::Away, id)) => {
                    ...
                    let c = w
                        ...
                        .unwrap();
                    (if lite { w.ball.vel } else { c.vel }, 0.0)
                }
        ...
        for &i in &mine {
            ...
            if let Some((bvel, decel)) = chase {
                slot_for[p.id.0 as usize - 1] =
                    intercept_point(p.pos, ball, bvel, decel, &params, FIXED_DT, lite);
                continue;
            }
            if lite {
                // Fixed spokes: straight on, and +/-50 degrees. No greedy
                // assignment, no 16-way search -- a graph gets one slot each.
                let rank = mine.iter().position(|&j| j == i).unwrap_or(0);
                let off = [0.0f32, 50.0, -50.0, 110.0][rank.min(3)].to_radians();
                let base = to_goal.y.atan2(to_goal.x) + off;
                slot_for[p.id.0 as usize - 1] =
                    anchor + Vec2::new(base.cos(), base.sin()) * spacing;
                continue;
            }
            ...
            for (k, d) in dirs.iter().enumerate() {
                ...
                let exposure = freedom(lite, spot, p.stamina, &threats, hold, r_int);
        ...
        for &i in &mine {
            ...
            let f = freedom(lite, end_pos[s], p.stamina, &threats, hold, r_int);
