...
struct Args {
    ...
    wall_secs: f32,
    quiet: bool,
    /// Path to a graph .txt to drive the GK side instead of the default
    /// cascade (e.g. data/titanium/Titanium.txt — the real Python
    /// deliverable).
    gk_graph: Option<PathBuf>,
    /// Path to a graph .txt to drive the attacker side instead of the
    /// default cascade (e.g. AIA.txt, to test a real opponent striker
    /// in isolation).
    atk_graph: Option<PathBuf>,
}
...
fn parse_args(argv: &[String]) -> Result<Args, String> {
    ...
    while i < argv.len() {
        match argv[i].as_str() {
            "-h" | "--help" => {
                eprintln!(
                    "titanium_drill — Scenario 1 (atk center+ball, GK pre-set on cover; \
                     ...
                     the default cascade\n\
                     --atk PATH   drive the attacker with this graph .txt instead of \
                     the default cascade (e.g. a real AIA.txt striker)\n\
                     --quiet"
                );
                std::process::exit(0);
            }
            ...
            "--atk" => {
                i += 1;
                atk_graph = Some(PathBuf::from(argv.get(i).ok_or("--atk needs a path")?));
            }
            "--quiet" => quiet = true,
...
fn run_trial(
    ...
    gk_graph: &Option<PathBuf>,
    atk_graph: &Option<PathBuf>,
) -> Result<(RoundResult, f32, f32, bool, TrialDiagnostics), String> {
    ...
    let mut atk_brain: Box<dyn TeamBrain> = build_gk_brain(atk_graph)?;
...
fn main() -> ExitCode {
    ...
    for i in 0..args.trials {
        ...
        let (result, t, max_ch, released, diag) = match run_trial(
            params.clone(),
            attack_home,
            z_bias,
            args.secs,
